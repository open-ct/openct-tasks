function initTask(subTask) {
   var idPipe = 0;
   var solLabelAttr = {"font-size": 20, "font-weight":"bold"};
      ...
      for(var iRow = 0; iRow < wheels.length; iRow++){
         ...
         for(var iCol = 0; iCol < wheels[iRow].length; iCol++){
            ...
            if(wheel){
               ...
               var rotation = 0; // answer.rotation[iRow][iCol];
   ...
   function initHandle() {
      ...
      if(level == "hard"){
         ...
         paper.text(xLabel-wheelSize,yLabel+wheelSize/2,taskStrings.actionHandleLeft).attr(solLabelAttr);
         paper.text(xLabel+wheelSize,yLabel+wheelSize/2,taskStrings.actionHandleRight).attr(solLabelAttr);
      }else{
         ...
         paper.text(xLabel,yLabel,taskStrings.actionHandleUnique).attr(solLabelAttr);
      }
   };
   ...
   function drawWheel(type,x,y,lowered) {
      ...
      //}
      idPipe++;
      var number = paper.text(x,y,idPipe).attr(solLabelAttr);
      return paper.set(circle,number); //return paper.set(circle,pipe);
   };
