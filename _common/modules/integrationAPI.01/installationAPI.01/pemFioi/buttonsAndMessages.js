function setCookie(cookieName) {
    ...
    expdate.setTime(expdate.getTime() + 60 * 60 * 60 * 1000); //时间单位毫秒
    document.cookie = cookieName + "=" + true + ";expires=" + expdate.toGMTString() + ";path=/";
}
    ...
    window.displayHelper = {
        ...
        showValidatePopup: function(gradedLevel) {
            ...
            if ((gradedLevel == undefined) && (this.graderScore >= this.taskParams.maxScore - 0.001)) {
                ...
                if (nextLevel) {
                    ...
                    if (!threshold) {
                        if (gradedLevel == "medium") { threshold = this.thresholdMedium; } else if (gradedLevel == "easy") { threshold = this.thresholdEasy; }
                    }
        ...
        getFullFeedbackWithLevelsSavedMessage: function() {
            ...
            if (!answerExists) {
                ...
                if (this.hideScoreDetails) {} else if (this.hasSolution) {
        ...
        updateMessages: function() {
            ...
            if (this.showScore) {
                ...
                switch (taskMode) {
                    ...
                    case 'saved_unchanged':
                        if (!this.hasSolution) {
                            messages.saved = this.formatTranslation(this.strings.answerSavedModifyOrCancelIt, ["<a href='#' onclick=\"platform.validate('cancel', function(){}); return false;\" " + disabledStr + ">" + this.strings.cancelIt + "</a>"]);
                        } else {
                            messages.saved = this.formatTranslation(this.strings.answerNotSavedContestOver, ["<a href='#' onclick=\"displayHelper.validate('cancel'); return false;\" " + disabledStr + ">" + this.strings.reloadSubmittedAnswer + "</a>"]);
                        }
                        break;
                    case 'saved_changed':
